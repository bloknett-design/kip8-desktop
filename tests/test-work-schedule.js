...
describe('График работы — WorkSchedule', () => {
    ...
    describe('Task 256: SW версия v514 (история)', () => {
        ...
        test('v514 заменена актуальной версией', () => {
            assertTrue(sw.indexOf("kipia-v414") !== -1,
                'Актуальная версия — kipia-v414 (Task 291)');
        });
        test('Старая версия v514 убрана', () => {
            assertTrue(sw.indexOf("kipia-v386") === -1,
    ...
    describe('Task 259: SW версия v517 (история)', () => {
        ...
        test('Старая версия v517 убрана', () => {
            assertTrue(sw.indexOf("kipia-v389") === -1,
    ...
    describe('Task 260: SW версия v518', () => {
        ...
        test('CACHE_VERSION = kipia-v414', () => {
            assertTrue(sw.indexOf("kipia-v414") !== -1,
                'CACHE_VERSION должен быть kipia-v414 (Task 290)');
        });
        test('Старая версия v517 убрана', () => {
            assertTrue(sw.indexOf("kipia-v389") === -1,
                'Старая v517 не должна остаться в sw.js');
    ...
    describe('Task 269: SW версия v524', () => {
        ...
        test('Старая версия v523 убрана', () => {
            assertTrue(sw.indexOf("kipia-v395") === -1,
    ...
    describe('Task 272: SW версия v526', () => {
        ...
        test('Старая версия v525 убрана', () => {
            assertTrue(sw.indexOf("kipia-v397") === -1,
    ...
    describe('Task 274: SW версия v527 (история)', () => {
        ...
        test('Старая версия v527 убрана', () => {
            assertTrue(sw.indexOf("kipia-v399") === -1,
...
describe('Task 298 — коды статусов Т-12/Т-13: клиентские инварианты', () => {
    ...
    test('SW: кэш поднят до kipia-v414 (Task 298)', () => {
        const sw = fs.readFileSync(path.join(__dirname, '..', 'sw.js'), 'utf8');
        assertTrue(sw.indexOf("CACHE_VERSION = 'kipia-v414'") !== -1,
...
describe('Task 307 — вкладка «Сотрудники» удалена, кнопка «+ Сотрудник» в тулбаре', () => {
    ...
    test('SW: кэш поднят до kipia-v414 (Task 309; история: v547 — Task 308)', () => {
        assertTrue(sw.indexOf("CACHE_VERSION = 'kipia-v414'") !== -1,
            'CACHE_VERSION = kipia-v414');
    });
});
...
describe('Task 308 — вкладки «Инструктажи»/«Отпуска» удалены, кнопка «+ Отпуск» в тулбаре', () => {
    ...
    test('HTML: кнопка «+ Отпуск» в тулбаре над шахматкой', () => {
        ...
        assertTrue(ws.indexOf('id="wsVacBtn"') !== -1 &&
                   ws.indexOf('ws-toolbar-main') !== -1,
            'кнопка внутри бара над шахматкой');
    });
    ...
    test('JS: видимость кнопки — init() по _canEdit (как «Сформировать»)', () => {
        ...
        assertTrue(init.indexOf("var vacBtn = document.getElementById('wsVacBtn');") !== -1 &&
                   init.indexOf('vacBtn.hidden = !this._canEdit;') !== -1,
            'init() показывает «+ Отпуск» только ролям с правом правки');
    });
    ...
    test('JS: submitVacationForm перезагружает шахматку (loadGrid)', () => {
        ...
        assertTrue(block.indexOf('self.loadGrid();') !== -1,
            'после добавления — loadGrid() (план «ОТ» в пустых ячейках)');
    ...
    test('SW: кэш поднят до kipia-v414 (Task 308)', () => {
        assertTrue(sw.indexOf("CACHE_VERSION = 'kipia-v414'") !== -1,
            'CACHE_VERSION = kipia-v414');
        assertTrue(sw.indexOf("CACHE_VERSION = 'kipia-v412'") === -1,

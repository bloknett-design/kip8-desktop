...
// Task 360 — печать табеля: под шахматкой список мероприятий месяца
// (столбик) и перечень кодов ТОЛЬКО этого месяца (столбик)
require('./test-task360.js');
// Task 361 — печать: колонка «Сотрудник» по ширине текста (canvas
// measureText, кламп 24–48mm), дни делят остаток поровну, шрифты
// листа увеличены, бейджи мероприятий в ячейках возвращены
require('./test-task361.js');
// Task 362 — печать: мероприятия только печатаемого вида (сменный/
// дневной), коды мероприятий — в перечень кодов («ПЗ» у дневного
// персонала без записи сетки)
...
// к жёлтому (#ffc400 / #cc9900, было #f5a623 / #c96e00) — синхронно
// с kip8test
require('./test-task370.js');

// Task 371 — «Датчики температуры»: блок «Расчёт произвольных значений»
// (живой двусторонний пересчёт t ↔ R(t)/E(t), бисекция обратной задачи
// по монотонной НСХ, диапазоны НСХ с тостами)

...
УТОЧНЕНИЕ (Task 354, по фактам CI):
- Первый прогон тега (3538e8d): сборка success, verify-pe падение —
  ассерт x64 ждал PE OS 5.01, Electron 22 x64 объявляет 5.02 (минимум
  для x64; ia32 прошёл: i386+5.01). Фикс 6c54958: ожидания по архам
  {ia32: (5,1), x64: (5,2)} + усиленный тест-ассерт; тег пересоздан.
- Второй прогон: все джобы success. Релиз legacy-v2.1.8 (prerelease):
  KIPiA-Setup-2.1.8-lts-win7-ia32.exe (69.1 МБ) / -x64.exe (72.5 МБ) /
  универсальный -win7.exe (140.9 МБ). Сквозная верификация релизного
  ia32: KIPiA.exe = i386 + PE OS 5.01, asar {2.1.8-lts,
  kipiaWin7Legacy true} → запустится на 32-битной Win7/8.1.
- Известный остаток: build-win обычной сборки на 3538e8d — флэйк npm
